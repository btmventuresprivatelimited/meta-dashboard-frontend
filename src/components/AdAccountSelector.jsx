...
function sortRows(rows, { key, direction }) {
  return [...rows].sort((a, b) => {
    let av = METRIC_KEYS.has(key)
      ? toNumber(a[key])
      : String(a[key] ?? "").toLowerCase();
    let bv = METRIC_KEYS.has(key)
      ? toNumber(b[key])
...
function parseAudience(t) {
  ...
  const locations = [
    ...(geo.countries?.map(countryName) || []),
    ...(geo.regions?.map((r) => r.name) || []),
    ...(geo.cities?.map((c) => c.name) || []),
  ];
  const genders = t.genders;
  const gender =
    !genders || genders.length === 0 || genders.length === 2
      ? "All Genders"
      : genders[0] === 1
        ? "Male Only"
        : "Female Only";
  return {
    age: `${t.age_min || 13}–${t.age_max || 65}`,
    gender,
    locations,
    ...
    lookalikes: (t.flexible_spec || []).flatMap(
      (s) => s.lookalike_audience || [],
    ),
  };
}
...
const ChevUp = () => (
  <svg width="9" height="9" viewBox="0 0 9 9" fill="currentColor">
    <path d="M4.5 2L8.5 7H0.5L4.5 2Z" />
  </svg>
);
const ChevDown = () => (
  <svg width="9" height="9" viewBox="0 0 9 9" fill="currentColor">
    <path d="M4.5 7L0.5 2H8.5L4.5 7Z" />
  </svg>
);
const ChevBoth = () => (
  <svg width="9" height="9" viewBox="0 0 9 9" fill="currentColor" opacity=".3">
    <path d="M4.5 1.5L7 4H2L4.5 1.5Z" />
    <path d="M4.5 7.5L2 5H7L4.5 7.5Z" />
  </svg>
);
...
function SortableHeader({ label, sortKey, section, sortState, onSort, right }) {
  ...
  return (
    <th
      onClick={() => onSort(section, sortKey)}
      className={`sheets-th ${right ? "text-right" : "text-left"} ${active ? "active-col" : ""}`}
    >
...
function AudTag({ label, type }) {
  return (
    <span className={`aud-tag aud-${type}`} title={label}>
      {label}
    </span>
  );
}
...
function AudiencePanel({ adset }) {
  ...
  const budget =
    adset.daily_budget != null
      ? `${fmt("budget", adset.daily_budget)}/day`
      : adset.lifetime_budget != null
        ? `${fmt("budget", adset.lifetime_budget)} lifetime`
        : null;
  return (
      ...
      {(budget || adset.bid_strategy) && (
        <div className="aud-section">
          ...
          <div className="aud-kv-grid">
            {budget && (
              <>
                <span className="aud-kv-key">Budget</span>
                <span className="aud-kv-val">{budget}</span>
              </>
            )}
            {adset.bid_strategy && (
              <>
                <span className="aud-kv-key">Bid Strategy</span>
                <span className="aud-kv-val">
                  {adset.bid_strategy.replace(/_/g, " ")}
                </span>
              </>
            )}
            {adset.optimization_goal && (
              <>
                <span className="aud-kv-key">Optimize For</span>
                <span className="aud-kv-val">
      ...
      {aud ? (
        <>
          <div className="aud-section">
            ...
            <div className="aud-kv-grid">
              <span className="aud-kv-key">Age</span>
              <span className="aud-kv-val">{aud.age} yrs</span>
              <span className="aud-kv-key">Gender</span>
              <span className="aud-kv-val">{aud.gender}</span>
            </div>
          </div>
          {aud.locations.length > 0 && (
            <div className="aud-section">
              <div className="aud-section-label">📍 Locations</div>
              <div className="aud-tags">
                {aud.locations.map((l, i) => (
                  <AudTag key={i} label={l} type="geo" />
                ))}
              </div>
            </div>
          )}
          {aud.included.length > 0 && (
            <div className="aud-section">
              <div className="aud-section-label">✅ Included Audiences</div>
              <div className="aud-tags">
                {aud.included.map((a) => (
                  <AudTag key={a.id} label={a.name} type="included" />
                ))}
              </div>
            </div>
          )}
          {aud.excluded.length > 0 && (
            <div className="aud-section">
              <div className="aud-section-label">🚫 Excluded Audiences</div>
              <div className="aud-tags">
                {aud.excluded.map((a) => (
                  <AudTag key={a.id} label={a.name} type="excluded" />
                ))}
              </div>
            </div>
          )}
          {aud.lookalikes.length > 0 && (
            <div className="aud-section">
              <div className="aud-section-label">🔍 Lookalike Audiences</div>
              <div className="aud-tags">
                {aud.lookalikes.map((a, i) => (
                  <AudTag key={i} label={a.name || a.id} type="lookalike" />
                ))}
              </div>
            </div>
          )}
          {aud.interests.length > 0 && (
            <div className="aud-section">
              <div className="aud-section-label">💡 Interests</div>
              <div className="aud-tags">
                {aud.interests.map((a) => (
                  <AudTag key={a.id} label={a.name} type="interest" />
                ))}
              </div>
            </div>
          )}
          {aud.behaviors.length > 0 && (
            <div className="aud-section">
              <div className="aud-section-label">⚡ Behaviors</div>
              <div className="aud-tags">
                {aud.behaviors.map((a) => (
                  <AudTag key={a.id} label={a.name} type="behavior" />
                ))}
              </div>
            </div>
          )}
          {aud.locations.length === 0 &&
            ...
            aud.behaviors.length === 0 && (
              <p className="aud-hint">
                No detailed targeting configured (Advantage+ or broad audience)
              </p>
            )}
        </>
      ) : (
...
export default function MetaAccountSelectorPage() {
  ...
  const [adsetSort, setAdsetSort] = useState({
    key: "spend",
    direction: "desc",
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!ignore) setAccountsError(e.message || "Failed to load accounts");
      } finally {
    ...
    (async () => {
      try {
        ...
        const [meta, insights] = await Promise.all([
          fetchJson(
            `${API_BASE}/campaigns?accountId=${encodeURIComponent(selectedAccountId)}`,
          ),
          fetchJson(
            `${API_BASE}/insights/campaigns?accountId=${encodeURIComponent(selectedAccountId)}&from=${dateRange.from}&to=${dateRange.to}`,
          ),
        ]);
        if (ignore) return;
        ...
        for (const row of insights) {
          ...
          ex.spend += toNumber(row.spend);
          ex.reach += toNumber(row.reach);
          ex.impressions += toNumber(row.impressions);
          ex.clicks += toNumber(row.clicks);
          ex.ctr = toNumber(row.ctr);
          ex.cpc = toNumber(row.cpc);
          ex.roas =
            toNumber(row.purchase_roas) || toNumber(row.website_purchase_roas);
          insightMap.set(k, ex);
        }
        ...
        if (!ignore) setCampaignsError(e.message || "Failed to load campaigns");
      } finally {
        if (!ignore) setCampaignsLoading(false);
      }
    })();
    return () => {
      ignore = true;
    ...
    (async () => {
      try {
        ...
        const [adsetMeta, adsetInsights] = await Promise.all([
          fetchJson(
            `${API_BASE}/adsets/detail?campaignId=${encodeURIComponent(selectedCampaignId)}`,
          ),
          fetchJson(
            `${API_BASE}/insights/adsets?accountId=${encodeURIComponent(selectedAccountId)}&from=${dateRange.from}&to=${dateRange.to}&campaignId=${encodeURIComponent(selectedCampaignId)}`,
          ),
        ]);
        if (ignore) return;
        const insightMap = new Map();
        ...
        const merged = adsetMeta.map((a) => {
          ...
          return {
            ...
            daily_budget: a.daily_budget
              ? toNumber(a.daily_budget) / 100
              : null,
            lifetime_budget: a.lifetime_budget
              ? toNumber(a.lifetime_budget) / 100
              : null,
        ...
        if (!ignore) setAdsetsError(e.message || "Failed to load ad sets");
      } finally {
        if (!ignore) setAdsetsLoading(false);
      }
    })();
    return () => {
      ignore = true;
    ...
    (async () => {
      try {
        ...
        const rows = await fetchJson(
          `${API_BASE}/insights/ads?accountId=${encodeURIComponent(selectedAccountId)}&from=${dateRange.from}&to=${dateRange.to}&adsetId=${encodeURIComponent(selectedAdsetId)}`,
        );
        if (ignore) return;
        setAds(
          rows.map((r) => ({
            ...
            cpc: toNumber(r.cpc),
            roas:
              toNumber(r.purchase_roas) || toNumber(r.website_purchase_roas),
          })),
        );
      } catch (e) {
        if (!ignore) setAdsError(e.message || "Failed to load ads");
      } finally {
  ...
  const portfolios = useMemo(() => {
    ...
    for (const acc of accounts) {
      const prefix =
        acc.name?.split(/\s[-–]\s|\s\(/)[0]?.trim() || "All Accounts";
      if (!map.has(prefix))
        map.set(prefix, { id: prefix, name: prefix, accounts: [] });
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return portfolioAccounts.filter(
      (a) =>
  ...
  const selectedCampaign = useMemo(
    () => campaigns.find((c) => c.campaign_id === selectedCampaignId) ?? null,
    [campaigns, selectedCampaignId],
  );
  const selectedAdset = useMemo(
    () => adsets.find((a) => a.adset_id === selectedAdsetId) ?? null,
  ...
  const sortedCampaigns = useMemo(
    () => sortRows(campaigns, campaignSort),
    [campaigns, campaignSort],
  );
  const sortedAdsets = useMemo(
    () => sortRows(adsets, adsetSort),
    [adsets, adsetSort],
  );
  const sortedAds = useMemo(() => sortRows(ads, adSort), [ads, adSort]);
  ...
  const adsetTotals = useMemo(
    () => ({
      ...
      impressions: adsets.reduce((s, a) => s + a.impressions, 0),
      clicks: adsets.reduce((s, a) => s + a.clicks, 0),
    }),
    [adsets],
  );
  ...
  const handlePortfolioChange = useCallback(
    (portfolioId) => {
      ...
      const accs =
        portfolioId === "__all__"
          ? accounts
          : portfolios.find((p) => p.id === portfolioId)?.accounts || [];
          ...
          <div className="toolbar-controls">
            <div className="ctrl-group">
              ...
              >
                <option value="__all__">
                  All Portfolios ({accounts.length})
                </option>
                {portfolios.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.name} ({p.accounts.length})
                  </option>
                ))}
              </select>
            </div>
            <div className="ctrl-group">
              ...
              <input
                ...
                placeholder="Search…"
                className="ctrl-input search-input"
              />
              <select
            ...
            <div className="ctrl-group">
              ...
              <input
                ...
                className="ctrl-input date-input"
              />
        ...
        <div className="section-block">
          ...
          <div className="kpi-grid">
            <div className="kpi-card">
              <div className="kpi-value">{portfolioAccounts.length}</div>
              <div className="kpi-label">Ad Accounts</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-value">{campaigns.length}</div>
              <div className="kpi-label">Campaigns (loaded)</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-value metric-green">
                {fmt("spend", campaignTotals.spend)}
              </div>
              <div className="kpi-label">Total Spend</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-value">
                {fmt("impressions", campaignTotals.impressions)}
              </div>
              <div className="kpi-label">Impressions</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-value">
                {fmt("clicks", campaignTotals.clicks)}
              </div>
              <div className="kpi-label">Clicks</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-value">
                {fmt("reach", campaignTotals.reach)}
              </div>
          ...
          <div className="portfolio-accounts">
            {portfolioAccounts.map((acc) => (
              <button
                ...
                className={`account-pill ${acc.id === selectedAccountId ? "account-pill-active" : ""}`}
              >
        ...
        {selectedCampaign && (
          <div className="summary-strip">
            <div className="summary-item">
              ...
              <span className="summary-value">{selectedAccount?.name}</span>
            </div>
            <div className="summary-divider" />
            <div className="summary-item">
              <span className="summary-label">Campaign</span>
              <span className="summary-value campaign-name-strip">
            ...
            <div className="summary-item">
              ...
              <span className="summary-value metric-green">
                {fmt("spend", selectedCampaign.spend)}
              </span>
            </div>
            <div className="summary-item">
              <span className="summary-label">ROAS</span>
              <span className="summary-value metric-green">
                {fmt("roas", selectedCampaign.roas)}
              </span>
            </div>
            <div className="summary-item">
              <span className="summary-label">Clicks</span>
              <span className="summary-value">
                {fmt("clicks", selectedCampaign.clicks)}
              </span>
            </div>
            <div className="summary-item">
              <span className="summary-label">CTR</span>
              <span className="summary-value">
                {fmt("ctr", selectedCampaign.ctr)}
              </span>
            </div>
          </div>
        )}
            ...
            <table className="sheet-table">
              ...
              <tbody>
                {campaignsLoading ? (
                  ...
                  <EmptyRow colSpan={11} msg="No campaigns found." />
                ) : (
                  sortedCampaigns.map((c, i) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`data-row ${sel ? "selected-row" : ""}`}
                      >
                        ...
                        <td className="cell cell-num metric-spend">
                          {fmt("spend", c.spend)}
                        </td>
              ...
              {campaigns.length > 0 && (
                <tfoot>
                  <tr className="totals-row">
                    ...
                    <td className="cell totals-label" colSpan={3}>
                      Totals
                    </td>
                    <td className="cell cell-num">
                      {fmt("spend", campaignTotals.spend)}
              ...
              <table className="sheet-table">
                ...
                <tbody>
                  {adsetsLoading ? (
                    ...
                    <EmptyRow colSpan={12} msg="No ad sets found." />
                  ) : (
                    sortedAdsets.map((a, i) => {
                      const sel = a.adset_id === selectedAdsetId;
                      const budget =
                        a.daily_budget != null
                          ? `${fmt("budget", a.daily_budget)}/day`
                          : a.lifetime_budget != null
                            ? `${fmt("budget", a.lifetime_budget)} life`
                            : "—";
                      return (
                        <tr
                          key={a.adset_id}
                          onClick={() => setSelectedAdsetId(a.adset_id)}
                          className={`data-row ${sel ? "selected-row" : ""}`}
                        >
                          <td className="cell row-num">{i + 1}</td>
                          ...
                          <td className="cell">
                            <span className="bid-chip">
                              {a.bid_strategy?.replace(/_/g, " ") || "—"}
                            </span>
                          </td>
                          <td className="cell cell-num metric-spend">
                            {fmt("spend", a.spend)}
                ...
                {adsets.length > 0 && (
                  <tfoot>
                    <tr className="totals-row">
                      <td className="cell row-num" />
                      <td className="cell totals-label" colSpan={4}>
                        Totals
                      </td>
                      <td className="cell cell-num">
                        {fmt("spend", adsetTotals.spend)}
                      </td>
                      <td className="cell cell-num">
                        {fmt("reach", adsetTotals.reach)}
                      </td>
                      <td className="cell cell-num">
                        {fmt("impressions", adsetTotals.impressions)}
                      </td>
                      <td className="cell cell-num">
                        {fmt("clicks", adsetTotals.clicks)}
                      </td>
                      <td className="cell cell-num" />
                      <td className="cell cell-num" />
                      <td className="cell cell-num" />
          ...
          <div className="ads-audience-wrapper">
            <div className="sheet-wrapper ads-sheet">
              <div className="sheet-header">
                ...
                <span className="sheet-tab active-tab">
                  {selectedAdset?.adset_name || "Ad Set"}
                </span>
                ...
                <table className="sheet-table">
                  ...
                  <tbody>
                    {adsLoading ? (
                      ...
                      <EmptyRow
                        colSpan={9}
                        msg="No ads found for this ad set and date range."
                      />
                    ) : (
                      sortedAds.map((ad, i) => (
                        <tr
                          key={`${ad.ad_id}-${ad.adset_id}`}
                          className="data-row"
                        >
                          <td className="cell row-num">{i + 1}</td>
                          ...
                          <td className="cell cell-num">
                            {fmt("impressions", ad.impressions)}
                          </td>
                          <td className="cell cell-num">
                            {fmt("clicks", ad.clicks)}
                          </td>
                          <td className="cell cell-num">
                            {fmt("ctr", ad.ctr)}
                          </td>
                  ...
                  {ads.length > 0 && (
                    <tfoot>
                      <tr className="totals-row">
                        <td className="cell row-num" />
                        <td className="cell totals-label">Totals</td>
                        <td className="cell cell-num">
                          {fmt("spend", adTotals.spend)}
                        </td>
                        <td className="cell cell-num">
                          {fmt("reach", adTotals.reach)}
                        </td>
                        <td className="cell cell-num">
                          {fmt("impressions", adTotals.impressions)}
                        </td>
                        <td className="cell cell-num">
                          {fmt("clicks", adTotals.clicks)}
                        </td>
                        <td className="cell cell-num" />
                        <td className="cell cell-num" />
                        <td className="cell cell-num" />
